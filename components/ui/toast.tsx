...
// A module-level manager, so `toast.add(...)` works from anywhere the way the
// `sonner` import does — including outside React. `Toaster` hands this same
// manager to the provider, which is what connects the calls to the viewport.
...
function ToastViewport({ className, ...props }: ToastPrimitive.Viewport.Props) {
  return (
    <ToastPrimitive.Viewport
      ...
      className={cn(
        "pointer-events-none fixed inset-x-4 top-20 z-50 mx-auto w-auto max-w-sm outline-none sm:top-4 sm:right-4 sm:left-auto sm:mx-0 sm:w-full",
        className
      )}
      {...props}
    />
