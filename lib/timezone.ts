...
/**
 * Every bucket key from the range's start through the current in-progress one,
 * aligned in `timeZone`. Both dashboards fill their series from this list, so
 * an empty bucket is a zero rather than a gap.
 */
