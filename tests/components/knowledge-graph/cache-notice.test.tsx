...
import {
  CacheNotice,
  formatRemaining,
} from "@/components/knowledge-graph/cache-notice";

const cachedAt = "2026-09-14T06:32:00.000Z";

function renderNotice(props: { cachedAt: string; remainingSeconds: number }) {
  return render(<CacheNotice {...props} />);
}
...
describe("CacheNotice", () => {
  it("reports when the snapshot was written and how long it has left", () => {
    renderNotice({ cachedAt, remainingSeconds: 9720 });
  ...
  it("renders a different stamp for a different write time", () => {
    ...
    renderNotice({ cachedAt: "2026-09-14T09:05:00.000Z", remainingSeconds: 60 });
  ...
  it("stays inside the page's own box rather than portalling out of it", () => {
    ...
    expect(
      container.querySelector('[data-testid="kg-cache-notice"]')
    ).not.toBeNull();
  });

  // The corner it occupies is part of the graph: a drag or a zoom starting on
  // the notice has to reach the nodes underneath it.
  it("lets the pointer through to the graph", () => {
    renderNotice({ cachedAt, remainingSeconds: 9720 });

    expect(screen.getByTestId("kg-cache-notice")).toHaveClass(
      "pointer-events-none"
    );
  });
});
